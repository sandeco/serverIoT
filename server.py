...
def listar_sensores():
    dados = pd.read_csv('sensores.csv')

    sensores = dados['sensor'].drop_duplicates()

    json = sensores.to_json(orient = "records", date_format="iso")
    return json
...
@app.route("/listarsensores")
def flask_listar_sensores():
